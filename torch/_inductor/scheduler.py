...
class Scheduler:
    ...
    def score_fusion_memory(
        ...
        #
        # Only apply this for pointwise (non-reduction) operations. Reduction ops
        # (argmax, argmin, sum, etc.) have different memory access patterns and
        # fusing them based solely on buffer overlap can cause correctness issues.
        # Note: Other unsafe patterns (templates, foreach, aliasing) are already
        # filtered by can_fuse() before score_fusion_memory is called.
        #
        # Skip this optimization when either node is a template or could be fused
        # as prologue/epilogue - we don't want horizontal fusion to interfere with
        # template fusion decisions.
        if (
            score == 0
